...
func Test_updateProductCategoryHandler_ServeHTTP(t *testing.T) {
	...
	invalidRequestBody, err := json.Marshal(entity.UpdateProductCategoryDTO{
		ProductID: 1,
	})
	require.NoError(t, err)
	...
	mockUpdateCategoryUsecase := mocks.NewMockUpdateProductCategoryUsecase(ctrl)
	handler := NewUpdateProductCategoryHandler(mockUpdateCategoryUsecase)
	...
	}{
		{
			...
			prepare: func() {
				mockUpdateCategoryUsecase.
					EXPECT().
					UpdateCategory(gomock.Any(), gomock.Eq(dto)).
					Return(nil)
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, _ := v1.TestRequest(t, "", server, "POST", "/api/v1/product/updateCategory", tt.reqBody)
