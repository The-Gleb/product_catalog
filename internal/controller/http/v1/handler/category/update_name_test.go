...
func Test_updateCategoryNameHandler_ServeHTTP(t *testing.T) {
	...
	invalidRequestBody, err := json.Marshal(entity.UpdateCategoryNameDTO{
		CategoryID: 1,
	})
	require.NoError(t, err)
	...
	mockUpdateNameUsecase := mocks.NewMockUpdateCategoryNameUsecase(ctrl)
	handler := NewUpdateCategoryNameHandler(mockUpdateNameUsecase)
	...
	tests := []struct {
		...
		{
			name:    "negative, category with that ID not found",
			reqBody: validRequestBody,
			code:    400,
			prepare: func() {
				mockUpdateNameUsecase.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, _ := v1.TestRequest(t, "", server, "POST", "/api/v1/category/updateName", tt.reqBody)
